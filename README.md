...
- Check file owner and permissions with `ls -l` and adjust accordingly. Typically, your macOS user won't have write access to your drive out of the box so you need to write files as root or first prepare a target directory writable by everyone (`chmod 777`). For advanced users, you may override the default NFS export options with `--nfs-export-args` (e.g. to set anonymous user mapping to root and squash options with `rw,no_subtree_check,all_squash,anonuid=0,anongid=0,insecure`)
