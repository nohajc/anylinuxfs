...
- Check file owner and permissions with `ls -l` and adjust accordingly. Typically, your macOS user won't have write access to your drive out of the box so you need to write files as root or first prepare a target directory writable by everyone (`chmod 777`). Advanced users may achieve limited UID/GID mapping by overriding NFS export options (e.g. `--nfs-export-opts rw,no_subtree_check,all_squash,anonuid=0,anongid=0,insecure -n noowners` so that files appear to be owned by the macOS user)
